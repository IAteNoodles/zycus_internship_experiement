...
def score_schedule(project: Project, snap: ProgressSnapshot, as_of: date) -> SignalResult:
    ...
    raw_days_late = max((m.days_late(as_of) for m in overdue), default=0)
    days_late = min(raw_days_late, 730)
...
def score_budget(project: Project, snap: ProgressSnapshot) -> SignalResult:
    ...
    if variance > 25:
        ...
        score = 0
    if variance < -25:
        dir_text = f"({-variance:.0f} pt under budget)"
    elif variance > 25:
        dir_text = f"({variance:+.0f} pt over budget)"
    else:
        dir_text = f"({variance:+.0f} pt variance)"
    rationale = (
